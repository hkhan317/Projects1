...
char ** populate(char ** words, FILE *fptr, int *currentsize, int *maxsize)
{
    ...
    if(line == NULL) // didn't allocate memory
    {
      printf("The program failed");
      return 0;
	}
    while(fgets(line, LIMIT, fptr)) //while there are lines
    {
        if (*currentsize < *maxsize) //fscanf
        {
            words[*currentsize] = malloc(sizeof(char *) * LIMIT);
            strncpy(words[*currentsize], line, strlen(line)); 
            printf("%s", words[*currentsize]);
            ++*currentsize;
        }
...
char ** doubleIt(char **arr, int *maxsize)
{
    char ** temp = (char ** )malloc(sizeof(char *) * *maxsize * 2);
    for (int i = 0; i < *maxsize ; i++)
    {
        temp[i] = malloc(sizeof(char) * LIMIT);
        // copy arr[i] in temp[i]
        strncpy(temp[i], "  ", 1);
        temp[i][strlen(temp[i])] = '\0';
        strcpy(temp[i], arr[i]);
        // add \0 to temp[i]
